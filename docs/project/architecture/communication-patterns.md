...
| **将来: データ同期の一括最適化** | **非同期（Queue）** | **大量件数時のQueueによる負荷平準化** | **Queue Processing** | **非同期** | **結果整合性** |

> オフライン復旧時のデータ同期自体（クイズ・回答の同期）は中核機能であり、`04-offline-sync.md` の同期REST API（`PATCH .../offline-sync`）で既に提供する。上表の「将来: データ同期の一括最適化」は、件数が多い場合にQueueへ切り替える将来的な性能最適化のみを指す。

### 選定結果

**採用**: 同期通信（REST API）中心
**将来拡張**: 非同期通信（通知配信のイベント配信、大量データ同期時のMessage Queue最適化）
