...
出力先： reports/mutation-analysis/<テスト対象ファイル名>/mutant-< hash値 >.md
...
**⚠️ 重要**: `pnpm mutation-test` コマンドは**ユーザーが手動で実行**してください。

**理由**:

- Mutation Testは実行時間が長い（通常1-3分）
- システムリソースを大量に消費する
- Claude Codeが自動実行すると他の作業が阻害される可能性がある
...
Claude Codeは以下の作業のみを担当：

1. テストケースの分析・設計・実装
2. Lint/Typecheck/単体テストの実行
3. 結果レポートの分析・解釈
4. 改善提案の作成
