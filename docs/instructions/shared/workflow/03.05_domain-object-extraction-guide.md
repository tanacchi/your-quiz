...
このガイドは**ユースケース駆動抽出型**をメインアプローチとして採用しています。
...
| ID | ユースケース名 | 概要 | ビジネス価値 | 優先度 | 分析対象仕様 |
|----|---------------|------|-------------|--------|-------------|
| **UC1** | クイズ投稿 | 匿名ユーザーがクイズを作成・投稿 | 高（コンテンツ供給） | 🔴 高 | user-story#L11 |
| **UC2** | クイズ回答 | ユーザーがスワイプでクイズに回答 | 最高（核心価値） | 🔴 最高 | success#L3-L25 |
| **UC3** | クイズ承認 | 管理者が投稿クイズを承認・拒否 | 高（品質保証） | 🔴 高 | requirements#L20-L21 |
...
このアプローチにより、実用性と理解しやすさを両立した効果的なドメインオブジェクト抽出が実現されます。
...
このガイドは**ユースケース駆動抽出型**をメインアプローチとして特化しています。
...
1. **ビジネス価値優先**: 高価値ユースケースから順次実装
2. **段階的設計**: ユースケース実現に必要な最小限のドメインオブジェクトから開始
3. **実装指針明確**: 抽出されたドメインオブジェクトが直接実装に活用可能
4. **継続的改善**: ユースケース追加に伴うドメインモデルの進化

このアプローチにより、効率的で実用的なドメイン設計が実現されます。

---

**作成日**: 2025-01-03  
**更新日**: 2025-08-03  
**ガイド種別**: ドメイン抽出・設計 (ユースケース駆動特化版)
