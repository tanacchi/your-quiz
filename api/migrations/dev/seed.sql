...
INSERT INTO "FreeTextSolution" ("correct_answer", "matching_strategy", "case_sensitive") VALUES 
  ('Hello World', 'exact', 0),           -- id: 6
  ('console.log', 'partial', 0),         -- id: 7
  ('SELECT * FROM users', 'partial', 0), -- id: 8
...
INSERT INTO "Quiz" ("question", "answer_type", "solution_id", "explanation", "status", "creator_id", "created_at") VALUES 
  -- Boolean quizzes
  ('TypeScriptはJavaScriptのスーパーセットである', 'boolean', 1, 'TypeScriptはJavaScriptに静的型付けを追加した言語で、JavaScriptのスーパーセットです', 'approved', 1, CURRENT_TIMESTAMP),
  ('SQLではNULL値は0と等しい', 'boolean', 2, 'NULL値は0ではなく、未定義の値を表します。NULL値との比較は常にNULLを返します', 'approved', 1, CURRENT_TIMESTAMP),
  ('ReactはFacebookが開発したライブラリである', 'boolean', 3, 'ReactはFacebook（現Meta）が開発し、オープンソースとして公開されています', 'approved', 2, CURRENT_TIMESTAMP),
  ('CSSのflexboxはテーブルレイアウトよりも古い技術である', 'boolean', 4, 'Flexboxは比較的新しい技術で、テーブルレイアウトよりもはるかに新しいです', 'approved', 2, CURRENT_TIMESTAMP),
  ('Node.jsはブラウザでのみ動作する', 'boolean', 5, 'Node.jsはサーバーサイドでJavaScriptを実行するためのランタイム環境です', 'pending_approval', 3, CURRENT_TIMESTAMP),
  
  -- Free text quizzes
  ('プログラミングの世界で最初に表示される伝統的なメッセージは？', 'free_text', 6, '多くのプログラミング言語で最初に学ぶのは"Hello World"の表示です', 'approved', 2, CURRENT_TIMESTAMP),
  ('JavaScriptでコンソールに出力するために使用する関数名を答えてください', 'free_text', 7, 'console.log()がJavaScriptでの標準的なコンソール出力関数です', 'approved', 1, CURRENT_TIMESTAMP),
  ('すべてのユーザーを取得するSQLクエリを書いてください（テーブル名: users）', 'free_text', 8, 'SELECT文を使用してusersテーブルからすべてのレコードを取得します', 'approved', 3, CURRENT_TIMESTAMP),
  ('Reactで状態管理を行う基本的なフック名を答えてください', 'free_text', 9, 'useStateはReactの関数コンポーネントで状態を管理するための基本的なフックです', 'approved', 2, CURRENT_TIMESTAMP),
  ('JavaScriptで非同期処理を扱う現代的な構文を答えてください', 'free_text', 10, 'async/awaitはPromiseベースの非同期処理をより読みやすく書くための構文です', 'pending_approval', 4, CURRENT_TIMESTAMP),
  
  -- Single choice quizzes
  ('ES6で導入されたブロックスコープの変数宣言キーワードは？', 'single_choice', 11, 'letとconstがES6で導入されました。letは再代入可能、constは再代入不可です', 'approved', 2, CURRENT_TIMESTAMP),
  ('HTTPレスポンスでリクエスト成功を表すステータスコードは？', 'single_choice', 12, '200 OKはHTTPリクエストが正常に処理されたことを示します', 'approved', 3, CURRENT_TIMESTAMP),
  ('Reactで最も基本的な状態管理フックは？', 'single_choice', 13, 'useStateはReactの関数コンポーネントで状態を管理するための最も基本的なフックです', 'approved', 2, CURRENT_TIMESTAMP),
  ('SQLで最も一般的に使用される結合方法は？', 'single_choice', 14, 'INNER JOINは両方のテーブルに一致するレコードのみを返す、最も一般的な結合方法です', 'approved', 1, CURRENT_TIMESTAMP),
  ('CSSで現代的なレイアウトシステムとして広く使われているのは？', 'single_choice', 15, 'Flexboxは現代的なCSSレイアウトシステムとして広く採用されています', 'approved', 4, CURRENT_TIMESTAMP),
  
  -- Multiple choice quizzes
  ('以下のうち、JavaScriptフレームワーク/ライブラリはどれですか？（複数選択）', 'multiple_choice', 16, 'React、Vue.js、AngularはすべてJavaScriptフレームワークです。jQueryはライブラリですが、現代的なフレームワークではありません', 'approved', 1, CURRENT_TIMESTAMP),
  ('RESTful APIで使用される基本的なHTTPメソッドをすべて選んでください', 'multiple_choice', 17, 'GET、POST、PUT、DELETEはRESTful APIの基本的なCRUD操作に対応するHTTPメソッドです', 'approved', 3, CURRENT_TIMESTAMP);
...
INSERT INTO "QuizTag" ("quiz_id", "tag_id", "assigned_at") VALUES 
  ...
  -- console.log quiz
  (7, 5, CURRENT_TIMESTAMP), (7, 11, CURRENT_TIMESTAMP),
  -- SQL query quiz
  (8, 3, CURRENT_TIMESTAMP), (8, 9, CURRENT_TIMESTAMP), (8, 12, CURRENT_TIMESTAMP),
  -- useState quiz
  (9, 7, CURRENT_TIMESTAMP), (9, 14, CURRENT_TIMESTAMP), (9, 12, CURRENT_TIMESTAMP),
  -- async/await quiz
  (10, 5, CURRENT_TIMESTAMP), (10, 13, CURRENT_TIMESTAMP),
  -- ES6 variables quiz
  (11, 5, CURRENT_TIMESTAMP), (11, 12, CURRENT_TIMESTAMP),
  -- HTTP status quiz
  (12, 2, CURRENT_TIMESTAMP), (12, 12, CURRENT_TIMESTAMP),
  -- React hooks quiz
  (13, 7, CURRENT_TIMESTAMP), (13, 14, CURRENT_TIMESTAMP), (13, 12, CURRENT_TIMESTAMP),
  -- SQL joins quiz
  (14, 3, CURRENT_TIMESTAMP), (14, 9, CURRENT_TIMESTAMP), (14, 12, CURRENT_TIMESTAMP),
  -- CSS layout quiz
  (15, 2, CURRENT_TIMESTAMP), (15, 14, CURRENT_TIMESTAMP), (15, 12, CURRENT_TIMESTAMP),
  -- JS frameworks quiz
  (16, 5, CURRENT_TIMESTAMP), (16, 7, CURRENT_TIMESTAMP), (16, 2, CURRENT_TIMESTAMP), (16, 13, CURRENT_TIMESTAMP),
  -- HTTP methods quiz
  (17, 2, CURRENT_TIMESTAMP), (17, 15, CURRENT_TIMESTAMP), (17, 13, CURRENT_TIMESTAMP);

-- Create diverse sample decks
INSERT INTO "Deck" ("name", "description", "quiz_ids", "creator_id", "created_at") VALUES 
  ('JavaScript基礎', 'JavaScriptの基本的な概念とES6の機能', '[1,7,11,16]', 1, CURRENT_TIMESTAMP),
  ('React入門', 'Reactの基本的なフックと概念', '[3,9,13]', 2, CURRENT_TIMESTAMP),
  ('データベース基礎', 'SQLの基本的なクエリと概念', '[2,8,14]', 3, CURRENT_TIMESTAMP),
  ('Web開発総合', 'フロントエンドとバックエンドの基礎', '[3,4,12,15,17]', 1, CURRENT_TIMESTAMP),
  ('プログラミング入門', '初心者向けの幅広いプログラミング概念', '[1,2,6,7,11]', 4, CURRENT_TIMESTAMP);
