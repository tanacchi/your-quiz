...
export class D1QuizSummaryMapper {
   ...
   */
  static fromRow(row: QuizRow): ResultAsync<QuizSummary, AppError> {
    return ResultAsync.fromSafePromise(
      (async () => {
        ...
        if (quizSummaryResult.isErr()) {
          throw new InternalServerError(
            `Failed to create QuizSummary from row data: ${JSON.stringify(quizSummaryResult.error)}`,
          );
        }

        return quizSummaryResult.value;
      })(),
    );
  }
    ...
    return ResultAsync.fromSafePromise(
      (async () => {
        ...
        for (const [index, row] of rows.entries()) {
          const mappingResult = await D1QuizSummaryMapper.fromRow(row);
  ...
  /**
   * QuizSummaryエンティティをD1データベースの更新用データに変換
   *
   * @param quizSummary - QuizSummaryエンティティ
   * @returns D1データベース更新用データ、またはマッピングエラー
   */
  static toUpdateData(quizSummary: QuizSummary): ResultAsync<
    {
      question: string;
      answer_type: string;
    ...
    return ResultAsync.fromSafePromise(
      (async () => {
        ...
        return {
          ...
          answer_type: quizSummary.get("answerType"),
